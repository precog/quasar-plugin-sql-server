  ...
  def apply[F[_]: ConcurrentEffect](
    ...
    def loadCsv(bytes: InputStream, connection: java.sql.Connection)
      ...
      for {
        ...
        _ <- FC.delay(bulkCopy.setDestinationTableName("precogtest.dbo.intdata"))//unsafeObj.drop(1).dropRight(1)))
        ...
        _ <- FC.delay(bulkCSV.addColumnMetadata(1, "", doobie.enum.JdbcType.Integer.toInt, 0, 0))
    ...
    bytes => {
      Stream.resource(xa.connect(xa.kernel)) flatMap { connection =>
        ...
        bytes.onFinalize(ConcurrentEffect[F].delay(logger.debug(s"finished byte stream"))).observe(_.map(b => println(s"bytes: $b"))).through(fs2.io.toInputStream[F]).evalMap(doLoad(_, unwrapped).transact(xa))
