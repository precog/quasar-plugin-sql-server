    ...
    for {
      ...
      connectionString =
        ConnectionConfig.Optics.parameters
          .modify(DriverParameter.ApplicationIntent :: DriverParameter.SelectMethod :: DriverParameter.DelayLoadingJobs :: _)(cc)
          .jdbcUrl

      jdbcUrl <-
        Either.catchNonFatal(new URI(connectionString))
          .leftMap(_ => NonEmptyList.one("Malformed JDBC connection string, ensure any restricted characters are properly escaped"))
